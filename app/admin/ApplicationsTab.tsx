...
export default function ApplicationsTab({ applications }: { applications: Application[] }) {
  ...
  return (
    <div>
      <div className="flex gap-1 mb-4" style={{ background: "#282b34", padding: "2px" }}>
        {(["pending", "all"] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{
              ...
              color: filter === f ? "#15171e" : "rgba(237,236,234,0.4)",
            }}>
            {f === "pending" ? "待審" : "全部"}
          </button>
        ))}
      ...
      <div className="space-y-px">
        {list.map(app => {
          ...
          return (
            <div key={app.id} style={{ background: "#282b34", borderBottom: "1px solid rgba(255,255,255,0.04)", padding: "1rem" }}>
