...
import { encryptText, decryptText } from "@/lib/secure-text";
...
export async function submitApplication(formData: FormData) {
  ...
  // 密碼用 AES-256-GCM 加密後存 DB；通過審核時解密丟給 Supabase 建帳號、
  // 立刻把欄位清成 null，存放期間不會有明文。
  let encryptedPassword: string;
  try {
    encryptedPassword = encryptText(password);
  } catch (e) {
    console.error("[applications] encrypt password failed", e);
    return { error: "系統未正確設定，請聯絡管理員" };
  }
  ...
  const { error } = await admin
    .from("applications")
    .insert({ email, display_name, note, password: encryptedPassword });
...
export async function approveApplication(id: string) {
  ...
  if (!application.password) return { error: "此申請沒有密碼欄位（資料異常）" };

  // 解密密碼。新申請是 AES 加密，但若 DB 還有舊明文資料則 try-catch 退回原值。
  let plainPassword: string;
  try {
    plainPassword = decryptText(application.password);
  } catch {
    // 舊的未加密申請（< 1 週前）：直接當明文用
    plainPassword = application.password;
  }

  // 建 auth user（Supabase 內部會 bcrypt 雜湊密碼）
  ...
  // 寄通知信（純通知，沒有連結，學生用原本填的 email + 密碼登入）
