-- 報名時學生可以留一段備註給老師（選填、自由文字）。
-- 重建 RPC，多收 p_notes 參數。

ALTER TABLE registrations
  ADD COLUMN IF NOT EXISTS notes text;

DROP FUNCTION IF EXISTS register_for_course(text, text, text, text, text, uuid, uuid[], int);
...
BEGIN
  ...
  BEGIN
    INSERT INTO registrations (course_id, name, email, line_id, transfer_last4, user_id, session_ids, transfer_amount, notes)
...
REVOKE ALL ON FUNCTION register_for_course(text, text, text, text, text, uuid, uuid[], int, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION register_for_course(text, text, text, text, text, uuid, uuid[], int, text)
